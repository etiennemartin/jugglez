...
class HighScoreScene: SKScene {
    ...
    private func addScore(dificulty: String, mode: GameMode, score: String) {
        ...
        let scoreBg = SKShapeNode(rectOfSize: CGSize(width: size.width, height: 50))
        scoreBg.fillColor = SKColor.themeLightBackgroundColor()
        scoreBg.position = CGPoint(x: size.width / 2, y: _prevScorePos.y + 12)
        addChild(scoreBg)

        if GameScores.sharedInstance.isRecordNewForMode(mode) {
            // New high score
            let newBg = SKShapeNode(rect: CGRect(x: 0, y: 0, width: 34, height: 24), cornerRadius: 3)
            newBg.fillColor = SKColor.colorForGameMode(mode)
            newBg.strokeColor = SKColor.colorForGameMode(mode)
            newBg.position = CGPoint(x: scoreValue.position.x + 10, y: scoreValue.position.y)
            addChild(newBg)
            ...
            newLabel.position = CGPoint(x: (newBg.frame.size.width / 2) - 1, y: 5)
